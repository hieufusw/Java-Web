...
<html>
    ...
    <body>
        ...
        <nav class="navbar navbar-expand-lg navbar-dark bg-dark fixed-top">
            <div class="container">
                <a class="navbar-brand" href="<c:url value="/home"/>">Manufacturer Manager</a>
                                    ...
                                    <c:forEach var="item" items="${model.listResult}">
                                        <tr>
                                            ...
                                            <c:if test="${item.status == 1}">
                                                <td><a href="<c:url value="/manufacturer-update-status?id=${item.id}"/>">Active</a></td>
                                            </c:if>
                                            <c:if test="${item.status == 0}">
                                                <td><a href="<c:url value="/manufacturer-update-status?id=${item.id}"/>">DeActive</a></td>
                                            </c:if>
                                            <td><a href="<c:url value="/manufacturer-edit?id=${item.id}"/>">Update</a></td>
                                            <td><a href="<c:url value="/manufacturer-delete?id=${item.id}"/>">Delete</a></td>
                                        </tr>
                                    </c:forEach>                           
                                </tbody>
                            </table>
                            <div class="col-lg-12">
                                <a href="<c:url value="/manufacturer-edit"/>" class="btn btn-info" role="button">Add Manufacturer</a>
